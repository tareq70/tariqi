...
namespace tariqi.Application_Layer.Services
{
    public class VehiclesService : IVehiclesService
    {
        ...
        {
            if (dto.SeatsCount <= 0)
                throw new DomainValidationException("Seats count must be greater than zero.");

            var existingVehicle = await _vehiclesRepo.FindAsync(v => v.PlateNumber == dto.PlateNumber);
            if (existingVehicle.Any())
                throw new DomainValidationException("Vehicle with this plate number already exists.");
            ...
            if (role == "Driver")
            {
                ...
                if (driverVehicles.Any())
                    throw new DomainValidationException("Driver already has an active vehicle.");
            }
        ...
        public async Task DeleteVehicleAsync(int vehicleId, string currentUserId, string role)
        {
            ...
            if (!vehicle.IsActive)
                throw new DomainValidationException("Vehicle already deleted");
